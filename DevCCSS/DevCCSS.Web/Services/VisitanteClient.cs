...
namespace DevCCSS.Web.Services
{
    public class VisitanteClient
    {
        ...
        private async Task<T> EjecutarAsync<T>(Func<IVisitanteService, Task<T>> accion)
        {
            ...
            var factory = new ChannelFactory<IVisitanteService>(binding, endpoint);
        ...
        public Task<List<VisitanteDto>> ListarAsync() => EjecutarAsync(c => c.ListarAsync());
        public Task<VisitanteDto?> ObtenerPorIdAsync(int id) => EjecutarAsync(c => c.ObtenerPorIdAsync(id));
